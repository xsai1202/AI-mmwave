...
for xx = 1:2  
    %tic 
    [ rx_1,obj_1 ] = generate_environment_4lane(3,3,3,3,3,3,3,3 );
    rx_n = rx_1;
    obj_n = obj_1; 
    if rx_1(2) < -50 
        disp('start')
        for ss = 1:si_num      
            sys_param = struct('mirrorPlane',[24,50,50,24,50,0,24,-50,0,24,-50,50],'rpos',[],'src',[0,0,5],'obj',[]);

            [rx_n, obj_n] = mobilityModule(rx_n,10,0.01,obj_n,6);
            sys_param.rpos = rx_n;
            rxx(ss,:) = rx_n(1:2);  
